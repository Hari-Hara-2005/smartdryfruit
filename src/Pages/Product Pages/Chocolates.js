...
import { chocalates } from "../../utils/data";
...
const ProductCard = ({ product, isLoading }) => {
    ...
    const handleShareClick = async (product) => {
        ...
        const shareUrl = `https://smartdryfruitdryfruit.vercel.app/chocalates`;
    ...
    return (
        <>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'start', color: 'black', px: [2, 5, 4] }}>
                <Title color="#282828">Chocalates</Title>
            </Box>
            ...
            <Box sx={{ textAlign: 'center', px: [2, 3, 0], py: [5], zIndex: 30 }}>
                <Grid container spacing={6} justifyContent="center" alignItems="center">
                    {chocalates.map((chocalate) => (
                        <Grid item key={chocalate.id} xs={12} sm={6} md={3.8} lg={3.6}>
                            <ProductCard product={chocalate} isLoading={isLoading} />
                        </Grid>
                    ))}
                </Grid>
            </Box>
